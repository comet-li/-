...
export default {
  ...
  setup(props) {
    ...
    // 第一个功能:自动轮播图的逻辑 自动播放，暴露自动轮播属性，设置了就自动轮播
    let timer = null
    const autoPlayFn = () => {
      clearInterval(timer) // 每次开启定时器之前,清掉定时器  // 防止定时器重复添加
      // 自动播放,每隔多久切换索引
      timer = setInterval(() => {
        index.value++
    ...
    // 什么时间调用 autoPlayFn 进行自动播放
    // 需要监听sliders数据变化，判断如果有数据且autPlay是true
